...
void ShutdownMemoryTracker(void) {
  ...
  while (current) {
    LOG_ERROR("Memory Leak: %zu bytes allocated at %s:%d not freed.",
...
static void AddAllocation(void *ptr, size_t size, const char *file, int line) {
  ...
  LOG_DEBUG("Allocated %zu bytes at %p (%s:%d). Current memory: %zu bytes.",
            size, ptr, file, line, current_memory);
}
...
static void RemoveAllocation(void *ptr, const char *file, int line) {
  ...
  while (current) {
    if (current->ptr == ptr) {
      ...
      LOG_DEBUG("Freed %zu bytes from %p (%s:%d). Current memory: %zu bytes.",
                current->size, ptr, file, line, current_memory);
      free(current);
      return;
    }
    prev = current;
  ...
  LOG_WARN("Attempted to free untracked memory at %p (%s:%d).", ptr, file,
           line);
}
...
void *MT_Calloc(size_t num, size_t size, const char *file, int line) {
  ...
  if (ptr) {
    ...
    LOG_ERROR("calloc failed at %s:%d for %zu elements of size %zu bytes.",
              file, line, num, size);
  }
  return ptr;
}
