...
TextureOES* CreateTextureOES(GLenum target, GLenum internalformat, GLsizei width, GLsizei height, GLboolean mipmap) {
    TextureOES* tex = (TextureOES*)tracked_allocate_memory(sizeof(TextureOES));
    if (!tex) {
...
void TexImage2DOES(TextureOES* tex, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLenum format, GLenum textype, const void* pixels) {
    tex->internalformat = internalformat;
    tex->width = width;
    tex->height = height;
    LOG_DEBUG("TexImage2DOES: Set texture ID %u to level %d, size %dx%d.", tex->id, level, width, height);
}
