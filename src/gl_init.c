...
// Cleans up the OpenGL ES context if needed (empty here, could free resources later)
void GL_cleanup(void) {
    LOG_DEBUG("OpenGL ES cleanup completed.");
}

// This function sets up the viewport and adjusts the projection matrix accordingly
void GL_setupViewport(GLint x, GLint y, GLsizei width, GLsizei height) {
    ...
    // Adjust the projection matrix for a basic orthogonal projection (2D setup as default)
    ...
    LOG_DEBUG("Viewport set to: x=%d, y=%d, width=%d, height=%d", x, y, width, height);
}
