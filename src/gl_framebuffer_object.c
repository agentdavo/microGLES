...
static RenderbufferOES* create_renderbuffer(GLenum internalformat, GLsizei width, GLsizei height) {
    RenderbufferOES* rb = (RenderbufferOES*)tracked_malloc(sizeof(RenderbufferOES));
    if (!rb) {
    ...
    switch (internalformat) {
        ...
        case GL_DEPTH_COMPONENT16_OES:
            ...
            break;
        default:
            LOG_WARN("Unsupported internalformat 0x%X for RenderbufferOES.", internalformat);
            rb->red_size = rb->green_size = rb->blue_size = rb->alpha_size = rb->depth_size = rb->stencil_size = 0;
            break;
    }
    return rb;
}
...
static FramebufferOES* create_framebuffer(void) {
    FramebufferOES* fb = (FramebufferOES*)tracked_malloc(sizeof(FramebufferOES));
    if (!fb) {
...
GLboolean GL_APIENTRY glIsRenderbufferOES(GLuint renderbuffer) {
    for (GLint i = 0; i < gl_state.renderbuffer_count; ++i) {
        if (gl_state.renderbuffers[i]->id == renderbuffer) {
            LOG_DEBUG("glIsRenderbufferOES: Renderbuffer ID %u exists.", renderbuffer);
            return GL_TRUE;
        }
    }
    LOG_DEBUG("glIsRenderbufferOES: Renderbuffer ID %u does not exist.", renderbuffer);
    return GL_FALSE;
}
...
GL_API void GL_APIENTRY glBindRenderbufferOES(GLenum target, GLuint renderbuffer) {
    if (target != GL_RENDERBUFFER_OES) {
        LOG_ERROR("glBindRenderbufferOES: Invalid target 0x%X.", target);
    ...
    for (GLint i = 0; i < gl_state.renderbuffer_count; ++i) {
        if (gl_state.renderbuffers[i]->id == renderbuffer) {
            ...
            LOG_DEBUG("glBindRenderbufferOES: Bound renderbuffer ID %u.", renderbuffer);
            return;
        }
    }

    LOG_WARN("glBindRenderbufferOES: Renderbuffer ID %u not found.", renderbuffer);
...
GL_API void GL_APIENTRY glDeleteRenderbuffersOES(GLsizei n, const GLuint *renderbuffers) {
    for (GLsizei i = 0; i < n; ++i) {
        ...
        if (index != -1) {
            ...
            for (GLint fb = 0; fb < gl_state.framebuffer_count; ++fb) {
                FramebufferOES* framebuffer = gl_state.framebuffers[fb];
                if (framebuffer->color_attachment.type == ATTACHMENT_RENDERBUFFER &&
                    framebuffer->color_attachment.attachment.renderbuffer == rb) {
                    framebuffer->color_attachment.type = ATTACHMENT_NONE;
                    framebuffer->color_attachment.attachment.renderbuffer = NULL;
                }
                if (framebuffer->depth_attachment.type == ATTACHMENT_RENDERBUFFER &&
                    framebuffer->depth_attachment.attachment.renderbuffer == rb) {
                    framebuffer->depth_attachment.type = ATTACHMENT_NONE;
                    framebuffer->depth_attachment.attachment.renderbuffer = NULL;
                }
                if (framebuffer->stencil_attachment.type == ATTACHMENT_RENDERBUFFER &&
                    framebuffer->stencil_attachment.attachment.renderbuffer == rb) {
                    framebuffer->stencil_attachment.type = ATTACHMENT_NONE;
                    framebuffer->stencil_attachment.attachment.renderbuffer = NULL;
                }
            }
            tracked_free(rb, sizeof(RenderbufferOES));
            /* Remove from the array */
            for (GLint j = index; j < gl_state.renderbuffer_count - 1; ++j) {
                gl_state.renderbuffers[j] = gl_state.renderbuffers[j + 1];
            }
            gl_state.renderbuffer_count--;
            LOG_INFO("glDeleteRenderbuffersOES: Deleted renderbuffer ID %u.", rb_id);
        } else {
            LOG_WARN("glDeleteRenderbuffersOES: Renderbuffer ID %u does not exist.", rb_id);
        }
    }
}
...
GL_API void GL_APIENTRY glGenRenderbuffersOES(GLsizei n, GLuint *renderers) {
    for (GLsizei i = 0; i < n; ++i) {
        if (gl_state.renderbuffer_count >= MAX_RENDERBUFFERS) {
            LOG_WARN("glGenRenderbuffersOES: Maximum renderbuffer limit reached.");
            renderers[i] = 0;
            glSetError(GL_OUT_OF_MEMORY);
            continue;
        }
        ...
        RenderbufferOES* rb = create_renderbuffer(GL_RGBA4_OES, 0, 0); // Default format and size
        if (rb) {
            gl_state.renderbuffers[gl_state.renderbuffer_count++] = rb;
            renderers[i] = rb->id;
            LOG_DEBUG("glGenRenderbuffersOES: Generated renderbuffer ID %u.", rb->id);
        } else {
            renderers[i] = 0;
            LOG_ERROR("glGenRenderbuffersOES: Failed to generate renderbuffer.");
            glSetError(GL_OUT_OF_MEMORY);
        }
    }
}
...
GL_API void GL_APIENTRY glRenderbufferStorageOES(GLenum target, GLenum internalformat, GLsizei width, GLsizei height) {
    if (target != GL_RENDERBUFFER_OES) {
        LOG_ERROR("glRenderbufferStorageOES: Invalid target 0x%X.", target);
        glSetError(GL_INVALID_ENUM);
        return;
    }

    if (!gl_state.bound_renderbuffer) {
        LOG_ERROR("glRenderbufferStorageOES: No renderbuffer is currently bound.");
    ...
    switch (internalformat) {
        ...
        case GL_DEPTH_COMPONENT16_OES:
            ...
            break;
        default:
            LOG_WARN("glRenderbufferStorageOES: Unsupported internalformat 0x%X.", internalformat);
            glSetError(GL_INVALID_ENUM);
            break;
    }

    LOG_DEBUG("glRenderbufferStorageOES: Set storage for renderbuffer ID %u to internalformat=0x%X, width=%d, height=%d.",
...
GL_API void GL_APIENTRY glGetRenderbufferParameterivOES(GLenum target, GLenum pname, GLint *params) {
    if (target != GL_RENDERBUFFER_OES) {
        LOG_ERROR("glGetRenderbufferParameterivOES: Invalid target 0x%X.", target);
        glSetError(GL_INVALID_ENUM);
        return;
    }

    if (!gl_state.bound_renderbuffer) {
        LOG_ERROR("glGetRenderbufferParameterivOES: No renderbuffer is currently bound.");
        glSetError(GL_INVALID_OPERATION);
        return;
    }
    ...
    switch (pname) {
        ...
        case GL_RENDERBUFFER_STENCIL_SIZE_OES:
            ...
            break;
        default:
            LOG_ERROR("glGetRenderbufferParameterivOES: Invalid pname 0x%X.", pname);
            glSetError(GL_INVALID_ENUM);
            return;
    }

    LOG_DEBUG("glGetRenderbufferParameterivOES: Retrieved pname=0x%X for renderbuffer ID %u.", pname, gl_state.bound_renderbuffer->id);
}
...
GLboolean GL_APIENTRY glIsFramebufferOES(GLuint framebuffer) {
    for (GLint i = 0; i < gl_state.framebuffer_count; ++i) {
        if (gl_state.framebuffers[i]->id == framebuffer) {
            LOG_DEBUG("glIsFramebufferOES: Framebuffer ID %u exists.", framebuffer);
            return GL_TRUE;
        }
    }
    LOG_DEBUG("glIsFramebufferOES: Framebuffer ID %u does not exist.", framebuffer);
    return GL_FALSE;
}
...
GL_API void GL_APIENTRY glBindFramebufferOES(GLenum target, GLuint framebuffer) {
    ...
    if (framebuffer == 0) {
        ...
        LOG_DEBUG("glBindFramebufferOES: Bound to default framebuffer.");
        return;
    }
    ...
    for (GLint i = 0; i < gl_state.framebuffer_count; ++i) {
        if (gl_state.framebuffers[i]->id == framebuffer) {
            ...
            LOG_DEBUG("glBindFramebufferOES: Bound framebuffer ID %u.", framebuffer);
            return;
        }
    }

    LOG_WARN("glBindFramebufferOES: Framebuffer ID %u not found.", framebuffer);
    glSetError(GL_INVALID_VALUE);
}
...
GL_API void GL_APIENTRY glDeleteFramebuffersOES(GLsizei n, const GLuint *framebuffers) {
    for (GLsizei i = 0; i < n; ++i) {
        ...
        if (index != -1) {
            ...
            if (fb->color_attachment.type == ATTACHMENT_RENDERBUFFER) {
                fb->color_attachment.attachment.renderbuffer = NULL;
            }
            if (fb->depth_attachment.type == ATTACHMENT_RENDERBUFFER) {
                fb->depth_attachment.attachment.renderbuffer = NULL;
            }
            if (fb->stencil_attachment.type == ATTACHMENT_RENDERBUFFER) {
                fb->stencil_attachment.attachment.renderbuffer = NULL;
            }
            ...
            /* Remove from the array */
            for (GLint j = index; j < gl_state.framebuffer_count - 1; ++j) {
                gl_state.framebuffers[j] = gl_state.framebuffers[j + 1];
            }
            gl_state.framebuffer_count--;
            LOG_DEBUG("glDeleteFramebuffersOES: Deleted framebuffer ID %u.", fb_id);
        } else {
            LOG_WARN("glDeleteFramebuffersOES: Framebuffer ID %u does not exist.", fb_id);
            glSetError(GL_INVALID_VALUE);
        }
    }
}
...
GL_API void GL_APIENTRY glGenFramebuffersOES(GLsizei n, GLuint *framebuffers) {
    for (GLsizei i = 0; i < n; ++i) {
        if (gl_state.framebuffer_count >= MAX_FRAMEBUFFERS) {
            LOG_WARN("glGenFramebuffersOES: Maximum framebuffer limit reached.");
            framebuffers[i] = 0;
            glSetError(GL_OUT_OF_MEMORY);
            continue;
        }
        ...
        if (fb) {
            gl_state.framebuffers[gl_state.framebuffer_count++] = fb;
            framebuffers[i] = fb->id;
            LOG_DEBUG("glGenFramebuffersOES: Generated framebuffer ID %u.", fb->id);
        } else {
            framebuffers[i] = 0;
            LOG_ERROR("glGenFramebuffersOES: Failed to generate framebuffer.");
            glSetError(GL_OUT_OF_MEMORY);
        }
    }
}
...
GLenum GL_APIENTRY glCheckFramebufferStatusOES(GLenum target) {
    if (target != GL_FRAMEBUFFER_OES) {
        LOG_ERROR("glCheckFramebufferStatusOES: Invalid target 0x%X.", target);
        glSetError(GL_INVALID_ENUM);
    ...
    if (fb == NULL) {
        LOG_ERROR("glCheckFramebufferStatusOES: No framebuffer is currently bound.");
        glSetError(GL_INVALID_OPERATION);
        return GL_FRAMEBUFFER_UNSUPPORTED_OES;
    }
    ...
    if (fb->color_attachment.type == ATTACHMENT_NONE &&
        fb->depth_attachment.type == ATTACHMENT_NONE &&
        fb->stencil_attachment.type == ATTACHMENT_NONE) {
        LOG_DEBUG("glCheckFramebufferStatusOES: Framebuffer is incomplete - no attachments.");
        return GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT_OES;
    }

    /* Check for dimension consistency */
    GLsizei width = 0, height = 0;
    AttachmentType types_to_check[3] = { fb->color_attachment.type, fb->depth_attachment.type, fb->stencil_attachment.type };
    RenderbufferOES* renderbuffers_to_check[3] = { fb->color_attachment.attachment.renderbuffer,
                                                  fb->depth_attachment.attachment.renderbuffer,
                                                  fb->stencil_attachment.attachment.renderbuffer };
    TextureOES* textures_to_check[3] = { fb->color_attachment.attachment.texture,
                                         fb->depth_attachment.attachment.texture,
                                         fb->stencil_attachment.attachment.texture };
    for (int i = 0; i < 3; ++i) {
        if (types_to_check[i] == ATTACHMENT_RENDERBUFFER && renderbuffers_to_check[i]) {
            if (i == 0) { // Color attachment
                width = renderbuffers_to_check[i]->width;
                height = renderbuffers_to_check[i]->height;
            } else {
                /* Ensure dimensions match */
                if (renderbuffers_to_check[i]->width != width || renderbuffers_to_check[i]->height != height) {
                    LOG_DEBUG("glCheckFramebufferStatusOES: Framebuffer is incomplete - attachment dimensions do not match.");
                    return GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_OES;
                }
            }
        } else if (types_to_check[i] == ATTACHMENT_TEXTURE && textures_to_check[i]) {
            if (i == 0) { // Color attachment
                width = textures_to_check[i]->width;
                height = textures_to_check[i]->height;
            } else {
                if (textures_to_check[i]->width != width || textures_to_check[i]->height != height) {
                    LOG_DEBUG("glCheckFramebufferStatusOES: Framebuffer is incomplete - attachment dimensions do not match.");
                    return GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_OES;
                }
            }
...
GL_API void GL_APIENTRY glFramebufferRenderbufferOES(GLenum target, GLenum attachment, GLenum renderbuffertarget, GLuint renderbuffer) {
    if (target != GL_FRAMEBUFFER_OES) {
        LOG_ERROR("glFramebufferRenderbufferOES: Invalid target 0x%X.", target);
        glSetError(GL_INVALID_ENUM);
        return;
    }
    ...
    FramebufferOES* fb = gl_state.bound_framebuffer;
    if (fb == NULL) {
        LOG_ERROR("glFramebufferRenderbufferOES: No framebuffer is currently bound.");
        glSetError(GL_INVALID_OPERATION);
        return;
    }

    if (renderbuffertarget != GL_RENDERBUFFER_OES) {
        LOG_ERROR("glFramebufferRenderbufferOES: Invalid renderbuffertarget 0x%X.", renderbuffertarget);
        glSetError(GL_INVALID_ENUM);
        return;
    }
    ...
    if (renderbuffer != 0) {
        ...
        if (rb == NULL) {
            LOG_WARN("glFramebufferRenderbufferOES: Renderbuffer ID %u does not exist.", renderbuffer);
            glSetError(GL_INVALID_VALUE);
            return;
        }
    }
    ...
    switch (attachment) {
        ...
        case GL_STENCIL_ATTACHMENT_OES:
            ...
            break;
        default:
            LOG_ERROR("glFramebufferRenderbufferOES: Unsupported attachment 0x%X.", attachment);
            glSetError(GL_INVALID_ENUM);
            return;
    }

    LOG_DEBUG("glFramebufferRenderbufferOES: Attached renderbuffer ID %u to attachment 0x%X on framebuffer ID %u.",
...
GL_API void GL_APIENTRY glFramebufferTexture2DOES(GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level) {
    if (target != GL_FRAMEBUFFER_OES) {
        LOG_ERROR("glFramebufferTexture2DOES: Invalid target 0x%X.", target);
        glSetError(GL_INVALID_ENUM);
        return;
    }
    ...
    FramebufferOES* fb = gl_state.bound_framebuffer;
    if (fb == NULL) {
        LOG_ERROR("glFramebufferTexture2DOES: No framebuffer is currently bound.");
        glSetError(GL_INVALID_OPERATION);
        return;
    }
    ...
    if (textarget != GL_TEXTURE_2D) {
        LOG_ERROR("glFramebufferTexture2DOES: Unsupported textarget 0x%X.", textarget);
        glSetError(GL_INVALID_ENUM);
        return;
    }
    ...
    if (texture != 0) {
        ...
        if (tex == NULL) {
            LOG_WARN("glFramebufferTexture2DOES: Texture ID %u does not exist.", texture);
            glSetError(GL_INVALID_VALUE);
            return;
        }
    }
    ...
    switch (attachment) {
        ...
        case GL_STENCIL_ATTACHMENT_OES:
            ...
            break;
        default:
            LOG_ERROR("glFramebufferTexture2DOES: Unsupported attachment 0x%X.", attachment);
            glSetError(GL_INVALID_ENUM);
            return;
    }

    LOG_DEBUG("glFramebufferTexture2DOES: Attached texture ID %u to attachment 0x%X on framebuffer ID %u at level %d.",
...
GL_API void GL_APIENTRY glGetFramebufferAttachmentParameterivOES(GLenum target, GLenum attachment, GLenum pname, GLint *params) {
    if (target != GL_FRAMEBUFFER_OES) {
        LOG_ERROR("glGetFramebufferAttachmentParameterivOES: Invalid target 0x%X.", target);
        glSetError(GL_INVALID_ENUM);
        return;
    }
    ...
    FramebufferOES* fb = gl_state.bound_framebuffer;
    if (fb == NULL) {
        LOG_ERROR("glGetFramebufferAttachmentParameterivOES: No framebuffer is currently bound.");
        glSetError(GL_INVALID_OPERATION);
        return;
    }
    ...
    switch (attachment) {
        case GL_COLOR_ATTACHMENT0_OES:
            ...
            attachment_ptr.rb = fb->color_attachment.attachment.renderbuffer;
            attachment_ptr.tex = fb->color_attachment.attachment.texture;
            break;
        case GL_DEPTH_ATTACHMENT_OES:
            type = fb->depth_attachment.type;
            attachment_ptr.rb = fb->depth_attachment.attachment.renderbuffer;
            attachment_ptr.tex = fb->depth_attachment.attachment.texture;
            break;
        case GL_STENCIL_ATTACHMENT_OES:
            type = fb->stencil_attachment.type;
            attachment_ptr.rb = fb->stencil_attachment.attachment.renderbuffer;
            attachment_ptr.tex = fb->stencil_attachment.attachment.texture;
            break;
        default:
            LOG_ERROR("glGetFramebufferAttachmentParameterivOES: Unsupported attachment 0x%X.", attachment);
            glSetError(GL_INVALID_ENUM);
            return;
    }
    ...
    if (type == ATTACHMENT_NONE) {
        if (pname == GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE_OES) {
            ...
            LOG_ERROR("glGetFramebufferAttachmentParameterivOES: No object attached to attachment 0x%X.", attachment);
            glSetError(GL_INVALID_OPERATION);
        }
        return;
    }
    ...
    switch (pname) {
        case GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE_OES:
            params[0] = (type == ATTACHMENT_RENDERBUFFER) ? GL_RENDERBUFFER_OES : GL_TEXTURE_2D;
            break;
        ...
        case GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_CUBE_MAP_FACE_OES:
            ...
            break;
        default:
            LOG_ERROR("glGetFramebufferAttachmentParameterivOES: Unsupported pname 0x%X.", pname);
            glSetError(GL_INVALID_ENUM);
            return;
    }

    LOG_DEBUG("glGetFramebufferAttachmentParameterivOES: Retrieved pname=0x%X for attachment 0x%X on framebuffer ID %u.",
...
GL_API void GL_APIENTRY glGenerateMipmapOES(GLenum target) {
    if (target != GL_TEXTURE_2D) {
        LOG_ERROR("glGenerateMipmapOES: Invalid target 0x%X. Only GL_TEXTURE_2D is supported.", target);
        glSetError(GL_INVALID_ENUM);
        return;
    }
    ...
    FramebufferOES* fb = gl_state.bound_framebuffer;
    if (fb == NULL) {
        LOG_ERROR("glGenerateMipmapOES: No framebuffer is currently bound.");
        glSetError(GL_INVALID_OPERATION);
        return;
    }
    ...
    if (tex == NULL) {
        LOG_ERROR("glGenerateMipmapOES: No texture is attached to the framebuffer's color attachment.");
        glSetError(GL_INVALID_OPERATION);
        return;
    }
    ...
    if (!tex->mipmap_supported) {
        LOG_ERROR("glGenerateMipmapOES: Texture ID %u does not support mipmaps.", tex->id);
        glSetError(GL_INVALID_OPERATION);
        return;
    }
    ...
    LOG_DEBUG("glGenerateMipmapOES: Generated mipmaps for texture ID %u up to level %d.", tex->id, tex->current_level);
}
