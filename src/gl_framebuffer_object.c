...
GLboolean GL_APIENTRY glIsRenderbufferOES(GLuint renderbuffer) {
  for (GLint i = 0; i < gl_state.renderbuffer_count; ++i) {
    if (gl_state.renderbuffers[i]->id == renderbuffer) {
      LOG_DEBUG("glIsRenderbufferOES: Renderbuffer ID %u exists.",
                renderbuffer);
      return GL_TRUE;
    }
  }
...
GL_API void GL_APIENTRY glBindRenderbufferOES(GLenum target,
  ...
  for (GLint i = 0; i < gl_state.renderbuffer_count; ++i) {
    if (gl_state.renderbuffers[i]->id == renderbuffer) {
      ...
      LOG_DEBUG("glBindRenderbufferOES: Bound renderbuffer ID %u.",
                renderbuffer);
      return;
    }
  }
...
GL_API void GL_APIENTRY glDeleteRenderbuffersOES(GLsizei n,
  ...
  for (GLsizei i = 0; i < n; ++i) {
    ...
    if (index != -1) {
      ...
      LOG_WARN("glDeleteRenderbuffersOES: Renderbuffer ID %u does not exist.",
               rb_id);
    }
  }
}
...
GL_API void GL_APIENTRY glGenRenderbuffersOES(GLsizei n, GLuint *renderers) {
  for (GLsizei i = 0; i < n; ++i) {
    ...
    RenderbufferOES *rb =
        create_renderbuffer(GL_RGBA4_OES, 0, 0); // Default format and size
    if (rb) {
...
GL_API void GL_APIENTRY glRenderbufferStorageOES(GLenum target,
  ...
  case GL_DEPTH_COMPONENT16_OES:
    ...
    break;
  default:
    LOG_WARN("glRenderbufferStorageOES: Unsupported internalformat 0x%X.",
             internalformat);
    glSetError(GL_INVALID_ENUM);
    break;
  }

  LOG_DEBUG("glRenderbufferStorageOES: Set storage for renderbuffer ID %u to "
...
GL_API void GL_APIENTRY glDeleteFramebuffersOES(GLsizei n,
  ...
  for (GLsizei i = 0; i < n; ++i) {
    ...
    if (index != -1) {
      ...
      LOG_WARN("glDeleteFramebuffersOES: Framebuffer ID %u does not exist.",
               fb_id);
      glSetError(GL_INVALID_VALUE);
    }
  }
...
GLenum GL_APIENTRY glCheckFramebufferStatusOES(GLenum target) {
  ...
  if (fb->color_attachment.type == ATTACHMENT_NONE &&
      fb->depth_attachment.type == ATTACHMENT_NONE &&
      fb->stencil_attachment.type == ATTACHMENT_NONE) {
    LOG_DEBUG("glCheckFramebufferStatusOES: Framebuffer is incomplete - no "
              "attachments.");
    ...
    if (types_to_check[i] == ATTACHMENT_RENDERBUFFER &&
      ...
      if (i == 0) { // Color attachment
        ...
        if (renderbuffers_to_check[i]->width != width ||
            renderbuffers_to_check[i]->height != height) {
          LOG_DEBUG("glCheckFramebufferStatusOES: Framebuffer is incomplete - "
...
GL_API void GL_APIENTRY glFramebufferRenderbufferOES(GLenum target,
  ...
  if (renderbuffertarget != GL_RENDERBUFFER_OES) {
    LOG_ERROR("glFramebufferRenderbufferOES: Invalid renderbuffertarget 0x%X.",
              renderbuffertarget);
    glSetError(GL_INVALID_ENUM);
    return;
  }
  ...
  case GL_STENCIL_ATTACHMENT_OES:
    ...
    break;
  default:
    LOG_ERROR("glFramebufferRenderbufferOES: Unsupported attachment 0x%X.",
              attachment);
    glSetError(GL_INVALID_ENUM);
    return;
  }

  LOG_DEBUG("glFramebufferRenderbufferOES: Attached renderbuffer ID %u to "
...
GL_API void GL_APIENTRY glFramebufferTexture2DOES(GLenum target,
  ...
  if (textarget != GL_TEXTURE_2D) {
    LOG_ERROR("glFramebufferTexture2DOES: Unsupported textarget 0x%X.",
              textarget);
    glSetError(GL_INVALID_ENUM);
    return;
  }
  ...
  if (texture != 0) {
    ...
    if (tex == NULL) {
      LOG_WARN("glFramebufferTexture2DOES: Texture ID %u does not exist.",
               texture);
      glSetError(GL_INVALID_VALUE);
      return;
    }
  ...
  case GL_STENCIL_ATTACHMENT_OES:
    ...
    break;
  default:
    LOG_ERROR("glFramebufferTexture2DOES: Unsupported attachment 0x%X.",
              attachment);
    glSetError(GL_INVALID_ENUM);
    return;
  }

  LOG_DEBUG("glFramebufferTexture2DOES: Attached texture ID %u to attachment "
...
GL_API void GL_APIENTRY glGetFramebufferAttachmentParameterivOES(
  ...
  if (target != GL_FRAMEBUFFER_OES) {
    LOG_ERROR("glGetFramebufferAttachmentParameterivOES: Invalid target 0x%X.",
              target);
    glSetError(GL_INVALID_ENUM);
    return;
  }
  ...
  FramebufferOES *fb = gl_state.bound_framebuffer;
  if (fb == NULL) {
    LOG_ERROR("glGetFramebufferAttachmentParameterivOES: No framebuffer is "
              "currently bound.");
    glSetError(GL_INVALID_OPERATION);
    return;
  }
  ...
  case GL_STENCIL_ATTACHMENT_OES:
    ...
    break;
  default:
    LOG_ERROR("glGetFramebufferAttachmentParameterivOES: Unsupported "
              "attachment 0x%X.",
              attachment);
    glSetError(GL_INVALID_ENUM);
    return;
  ...
  if (type == ATTACHMENT_NONE) {
    if (pname == GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE_OES) {
      ...
      LOG_ERROR("glGetFramebufferAttachmentParameterivOES: No object attached "
                "to attachment 0x%X.",
                attachment);
      glSetError(GL_INVALID_OPERATION);
    }
  ...
  case GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE_OES:
    params[0] =
        (type == ATTACHMENT_RENDERBUFFER) ? GL_RENDERBUFFER_OES : GL_TEXTURE_2D;
    break;
  ...
  LOG_DEBUG("glGetFramebufferAttachmentParameterivOES: Retrieved pname=0x%X "
...
GL_API void GL_APIENTRY glGenerateMipmapOES(GLenum target) {
  if (target != GL_TEXTURE_2D) {
    LOG_ERROR("glGenerateMipmapOES: Invalid target 0x%X. Only GL_TEXTURE_2D is "
              "supported.",
              target);
    glSetError(GL_INVALID_ENUM);
    return;
  ...
  if (!tex->mipmap_supported) {
    LOG_ERROR("glGenerateMipmapOES: Texture ID %u does not support mipmaps.",
              tex->id);
    glSetError(GL_INVALID_OPERATION);
    return;
  }
  ...
  LOG_DEBUG("glGenerateMipmapOES: Generated mipmaps for texture ID %u up to "
            "level %d.",
